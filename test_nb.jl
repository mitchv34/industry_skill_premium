...
# ╔═╡ 5f8845e4-2181-11ed-2092-d758fc273ca1
begin
	using LinearAlgebra
	using DataFrames
	using CSV
...
# ╔═╡ c7854493-7b79-4d73-8af8-242c96c05ece
begin
	begin
...
# ╔═╡ a5bd4588-3978-43d6-9714-d941a93f4772
begin
...
function generateData(data::DataFrame)
	...
	function setParams(param::NTuple{4, Float64}, scale_params::NTuple{4, Float64}; δ_e::Float64=0.125, δ_s::Float64 =0.05)
	parameters = Params(
						α   = param[1],
						μ   = scale_params[1],    
						λ   = scale_params[2],    
...
begin
@bind param_0 PlutoUI.combine() do Child
	@htl("""
	<h6>Economic Parameters</h6>
	<ul>
	$(vcat(
		[@htl("<li>$(name): $(Child(Slider(0.01:0.01:1.0)))</li>")
		for name in ["α", "σ"]], 
		[@htl("<li>$(name): $(Child(Slider(-1.0:0.01:1.0)))</li>")
		for name in ["ρ"]],
		[@htl("<li>$(name): $(Child(Slider(0.01:0.01:1.0)))</li>")
		for name in ["η_ω"]],
	)
...
begin
@bind scale_0 PlutoUI.combine() do Child
	@htl("""
	<h6>Scaling Parameters</h6>
	<ul>
	$(vcat([
		@htl("<li>$(name): $(Child(Slider(0.01:0.01:1.0)))</li>")
		for name in ["λ", "μ"]
...
# scale_0 = [0.4, 0.4, scale_initial]
	...
	params = setParams(param_0, scale_0)
end
...
git-tree-sha1 = "c6be5a2efe56d944bfb9806d9d864f627b3e4a1e"
...
git-tree-sha1 = "710d2f477c52a891637692317c01cc14eb0c1a6e"
